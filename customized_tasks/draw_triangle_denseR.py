...
class DrawTriangleEnv(BaseEnv):
    ...
    **Success Conditions:**
    ...
    def compute_dense_reward(self, obs=None, action=None, info=None):
        """
        Dense reward for DrawTriangle-denseR
        Combines:
        1. Point coverage reward
        2. Penalty for brush far from paper z-plane
        3. Final completion reward
        ...
        if self.draw_step > 0:
            ...
            z_mask = current_dot[:, :, 2] < 0  # dot 未出现在画布上

            # 计算 xy 平面距离参考点
            dist = torch.sqrt(
                torch.sum(
                    (current_dot[:, :, None, :2] - self.triangles[:, None, :, :2]) ** 2,
                    dim=-1
                )
            )
            ...
            # 点覆盖奖励
            alpha = 1.0
            reward += alpha * new_covered / self.triangles.shape[1]  # 归一化

            # z-plane 距离惩罚
            brush_z = self.agent.tcp.pose.p[:, 2]
            paper_z = self.CANVAS_THICKNESS
            beta = 0.5
            distance_penalty = (brush_z - paper_z).clamp(min=0)
            reward -= beta * distance_penalty

            # 最终完成奖励（覆盖率大于 95%）
            coverage = self.ref_dist.float().sum(dim=1) / self.ref_dist.shape[1]
            success_mask = coverage >= 0.95
            reward[success_mask] += 5.0  # 最终完成奖励
